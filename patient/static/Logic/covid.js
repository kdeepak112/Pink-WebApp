...
myApp.controller("covidControl", function ($scope, $http) {
  ...
  $http.get(world_url).then(
    (world_response) => {
      ...
      $scope.countryData = $scope.world.Countries;
      ...
      $scope.world_assign = function (x) {
        let worldLen = Object.keys($scope.world.Countries).length;
        for (let i = 0; i < worldLen; i++) {
          if ($scope.world.Countries[i].Country == x) {
            $scope.total_confirmed = $scope.world.Countries[i].TotalConfirmed;
            $scope.new_confirmed = $scope.world.Countries[i].NewConfirmed;
            $scope.total_recovered = $scope.world.Countries[i].TotalRecovered;
            $scope.total_deaths = $scope.world.Countries[i].TotalDeaths;
            break;
          }
        }
      };
  ...
  $scope.showWorld = true;
  $scope.showData = false;
  $scope.text = 'Covid data for Countries';
  $scope.showWorldData = function () {
    if ($scope.showWorld == true) {
      $scope.showWorld = false;
      $scope.showData = true;
      $scope.text = 'Covid data for world'
    } else {
      $scope.showWorld = true;
      $scope.showData = false;
      $scope.text = 'Covid data for Countries';
