...
myApp.controller("confirmthisapp", function ($scope, $http, $interval,$timeout) {
  ...
  $scope.messages = [];
  $scope.docmssg = [];
  $http.get("/PatientMessageApi").then(
    (api) => {
      console.log(api.data);
      let oldLen = Object.keys(api.data).length;
      for (i = 0; i < oldLen; i++) {
        if (
          $scope.PName == api.data[i].to_patient &&
          $scope.DName == api.data[i].from_doctor 
        ...
        $http.get("/PatientMessageApi").then(
          (api) => {
            ...
            if (Len > oldLen) {
              console.log("yes");
              if (
                $scope.PName == api.data[Len - 1].to_patient &&
                $scope.DName == api.data[Len - 1].from_doctor 
              ) {
                $scope.messages.push({
